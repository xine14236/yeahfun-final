...
router.get('/', async function (req, res) {
  ...
  conditions[0] = name_like ? `store.name LIKE '%${name_like}%'` : ''

  // 價格
  const normal_price_gte = Number(req.query.normal_price_gte) || 0 // 最小價格
  const normal_price_lte = Number(req.query.normal_price_lte) || 10000 // 最大價格
  conditions[2] = `normal_price BETWEEN ${normal_price_gte} AND ${normal_price_lte}`
