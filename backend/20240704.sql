...
--
-- 使用資料表自動遞增(AUTO_INCREMENT) `blog_tag`
--
ALTER TABLE `blog_tag`
  MODIFY `id` int NOT NULL AUTO_INCREMENT;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `blog_tag_name`
--
ALTER TABLE `blog_tag_name`
  MODIFY `id` int NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `blog_tag`
  ADD CONSTRAINT `blog_tag_ibfk_1` FOREIGN KEY (`blog_id`) REFERENCES `blog` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `blog_tag_ibfk_2` FOREIGN KEY (`tag_id`) REFERENCES `blog_tag_name` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
